...
companySchema.pre('findOneAndUpdate', async function (next, opts) {
	if (this.logo || this.certificate) {
		const docToUpdate = await this.model.findOne(this.getQuery())
		const now = Date.now().toString(16)
		const manageupload = await S3Helper.upload(this.logo, `${now}logo`)
		const manageupload2 = await S3Helper.upload(this.certificate, `${now}cert`).catch(
			async () => {
		...
		if (manageupload && manageupload2) {
			...
			const oldKey2 = docToUpdate.certificate.key
			await S3Helper.delete(oldKey)
			await S3Helper.delete(oldKey2)
			next()
